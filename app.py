...
def index():
    ...
    #---------------------GRAFICO DE AREA--------------
    renewables_data = pd.read_csv('static/archivo/02 modern-renewable-energy-consumption.csv')
    renewables_data=renewables_data[renewables_data['Entity']=='World']
    renewables_data['Total Renewable Energy'] = (renewables_data['Geo Biomass Other - TWh']+renewables_data['Solar Generation - TWh']+renewables_data['Wind Generation - TWh']+renewables_data['Hydro Generation - TWh'])

    fig, ax = plt.subplots(figsize=(6,5))
    ax.fill_between(renewables_data['Year'], renewables_data['Total Renewable Energy'], color='green', alpha=0.5, label='Energía Ronovable')

    conventional_data = pd.DataFrame({
        'Year': renewables_data['Year'],
        'Total Conventional Energy':[1000]*len(renewables_data['Year'])
    })
    ...
    with open('static/archivo/data_pagina.csv', newline='', encoding='utf-8') as csvfile:
        ...
        data = [row for row in reader] # lo metemos en una lista
